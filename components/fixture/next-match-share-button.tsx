...
const DEFAULT_SHARE_PLATE_PALETTE: SharePlatePalette = {
  accent: "#f4ce0f",
  accentBorder: "rgba(244,206,15,0.62)",
...
function drawTimePill(
  ...
  context.save();
  context.fillStyle = palette.pillBackground;
  context.strokeStyle = palette.accentBorder;
  context.lineWidth = 3;
...
async function drawMatchup(
  ...
  const teamNameFontSize = getSharedTeamNameFontSize(
    context,
    [localDisplayName, visitorDisplayName],
    compact ? 24 : 28,
    teamColumnWidth,
    compact ? 18 : 21,
  );
...
function drawPlateTeamBlock(
  ...
  const lineHeight = Math.round(nameFontSize * 1.18);
  const lines = getClampedLines(
    context,
    label.toUpperCase(),
    maxWidth,
    2,
    `500 ${nameFontSize}px Arial, sans-serif`,
  );
  ...
  context.fillStyle = palette.roleBackground;
  context.strokeStyle = palette.roleStroke;
  context.lineWidth = 2;
